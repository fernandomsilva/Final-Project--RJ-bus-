...
def create_hdf5(in_dir, out_dir):
    ...
    lines_dic = {}
    for i,el in enumerate(list(csv.reader(open('lines_def.txt', 'rb'), delimiter=';'))):
        lines_dic[el[0]] = i
    ...
    for fp in files:
        data = list(csv.reader(open(fp, 'rb'), delimiter=';'))
        if data[0][2] in lines_dic: #only goes if the line is in the definition
            if data[0][2] =='636':
                pass
            ...
            if len(range(time_size,data_size,stride))>0:
                ...
                for j in range(time_size,data_size,stride):
                    ...
                    for k in range(time_size):
                        item = data[j-k]
                        timestamp = dateutil.parser.parse(item[11]) #the start date starts at 0 minutes of the hour
                        data4D[i,0,0,k] = (float(item[9])-lat_m)/0.1#/0.5 #divide by 0.1 in order to get larger numbers
                        data4D[i,1,0,k] = (float(item[10])-long_m)/0.1#/0.5  #divide by 0.1 in order to get larger numbers
                    ...
                    label2D[i,0] = lines_dic[item[2]] #Get only the last ground_truth
            ...
            pass
#train_in_dir = '/work/rfn216/bus_rio/Splitted_Data/'
#train_out_dir = '/work/rfn216/bus_rio/hdf5/'

train_in_dir = '/home/rfn/NYU/BusRio/Splitted_Data_Small/'
